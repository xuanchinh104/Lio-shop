...
(function () {
    ...
    function getPillarAtPositionY(pillars, positionX) {
        for (var i = 0, len = pillars.length; i < len; i++) {
            ...
            if (positionX >= pillar.y && positionX <= pillar.y + pillar.height) return pillar;
        }
    ...
    function getPillarAtPosition(pillars, positionX) {
        for (var i = 0, len = pillars.length; i < len; i++) {
            ...
            if (positionX >= pillar.x && positionX <= pillar.x + pillar.width) return pillar;
        }
        ...
        function resizeColumn(direction) {
            ...
            for (var i = 0; i < cellsCount; i++) {
                ...
                CKEDITOR.tools.setTimeout(
                    function (leftCell, leftOldWidth) {
    ...
    function rowResizer(editor) {
        ...
        function resizeRow(direction) {
            ...
            for (var i = 0; i < cellsCount; i++) {
                ...
                // Defer the resizing to avoid any interference among cells.
                CKEDITOR.tools.setTimeout(
                    function (leftCell, leftOldHeight, tableHeight) {
                        // 1px is the minimum valid width (http://dev.ckeditor.com/ticket/11626).
                    ...
                    0,
                    this,
                    [leftCell, leftCell && getHeight(leftCell), !leftCell && getHeight(leftCell.img)]
                );
            }
        }
        ...
        resizer = CKEDITOR.dom.element.createFromHtml(
            '<div id="resizer" data-cke-temp=1 contenteditable=false unselectable=on ' +
        ...
        move = this.move = function (posX) {
            ...
            if (isResizing) {
                if (resizerNewPosition == upShiftBoundary || resizerNewPosition == downShiftBoundary) return 1;
