                ...
                if (c.info) {
                    ...
                    if (null !== b.$.tHead && 'row' != f && 'both' != f) {
                        ...
                        for (h = b.getElementsByTag('tbody').getItem(0); 0 < g.getChildCount(); ) {
                            l = g.getFirst();
                            for (k = 0; k < l.getChildCount(); k++)
                                (m = l.getChild(k)), m.type == CKEDITOR.NODE_ELEMENT && (m.renameNode('td'), m.removeAttribute('scope'));
                            h.append(l, !0);
                        }
                        g.remove();
                    }
                    if (!this.hasColumnHeaders && ('col' == f || 'both' == f))
                        for (g = 0; g < b.$.rows.length; g++)
                            (m = new CKEDITOR.dom.element(b.$.rows[g].cells[0])), m.renameNode('th'), m.setAttribute('scope', 'row');
                    if (this.hasColumnHeaders && 'col' != f && 'both' != f)
