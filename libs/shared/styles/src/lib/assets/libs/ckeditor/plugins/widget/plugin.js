...
(function () {
    ...
    Repository.prototype = {
        ...
        add: function (name, widgetDef) {
            ...
            if (widgetDef.template) widgetDef.template = new CKEDITOR.template(widgetDef.template);
        ...
        checkSelection: function () {
            ...
            while ((wrapper = walker.next())) updater.select(this.getByElement(wrapper));
        ...
        destroy: function (widget, offline) {
            if (this.widgetHoldingFocusedEditable === widget) setFocusedEditable(this, widget, null, offline);
        ...
        initOn: function (element, widgetDef, startupData) {
            ...
            else if (typeof widgetDef == 'string') widgetDef = this.registered[widgetDef];
        ...
        wrapElement: function (element, widgetName) {
            ...
            if (element instanceof CKEDITOR.dom.element) {
                ...
                if (wrapper && wrapper.type == CKEDITOR.NODE_ELEMENT && wrapper.data('cke-widget-wrapper')) return wrapper;
                ...
                if (wrapper && wrapper.type == CKEDITOR.NODE_ELEMENT && wrapper.attributes['data-cke-widget-wrapper']) return wrapper;
    ...
    Widget.prototype = {
        ...
        destroy: function (offline) {
            ...
            if (!offline) {
                if (this.element.data('cke-widget-keep-attr') == '0') this.element.removeAttribute('data-widget');
        ...
        initEditable: function (editableName, definition) {
            ...
            if (editable && editable.is(CKEDITOR.dtd.$editable)) {
                ...
                if (definition.pathName) editable.data('cke-display-name', definition.pathName);
    ...
    function NestedEditable(editor, element, config) {
        ...
        if (!CKEDITOR.dtd[this.getName()].p) this.enterMode = this.shiftEnterMode = CKEDITOR.ENTER_BR;
        else {
    ...
    function checkWidgets(evt) {
        ...
        for (i in instances) {
            ...
            if (instances[i].isReady() && !editable.contains(instances[i].wrapper)) this.destroy(instances[i], true);
        }
        ...
        if (options && options.focusInited && newInstances.length == 1) newInstances[0].focus();
    }
    ...
    function createEditableFilter(widgetName, editableName, editableDefinition) {
        if (!editableDefinition.allowedContent && !editableDefinition.disallowedContent) return null;
                ...
                // and wrapped and stop iterating in this branch.
                    ...
                    for (i = 0; i < upcastsLength; ++i) {
                        ...
                        if ((upcasted = upcast[0](element, data))) {
                            ...
                            if (upcasted instanceof CKEDITOR.htmlParser.element) element = upcasted;
    ...
    function getWrapperAttributes(inlineWidget, name) {
        return {
            ...
            class:
                'cke_widget_wrapper cke_widget_new cke_widget_' + (inlineWidget ? 'inline' : 'block') + (name ? ' cke_widget_' + name : ''),
        };
    }
    ...
    function setFocusedEditable(widgetsRepo, widget, editableElement, offline) {
        ...
        if (editableElement) {
            ...
            if (editableInstance.filter) editor.setActiveFilter(editableInstance.filter);
            editor.setActiveEnterMode(editableInstance.enterMode, editableInstance.shiftEnterMode);
        } else {
            if (!offline) widget.focusedEditable.removeClass('cke_widget_editable_focused');
                        ...
                        lookups: {
                            ...
                            default: function (el) {
                                ...
                                if (nestedEditable) {
                                    ...
                                    if (widgetsRepo.getByElement(nestedEditable) == draggedWidget) return;
                                    ...
                                    if (filter && draggedRequiredContent && !filter.check(draggedRequiredContent)) return;
                                }
    ...
    function setupMouseObserver(widgetsRepo) {
            ...
            editable.attachListener(evtRoot, 'mousedown', function (evt) {
                ...
                if (widget) {
                    ...
                    if (widget.inline && target.type == CKEDITOR.NODE_ELEMENT && target.hasAttribute('data-cke-widget-drag-handler')) {
                        ...
                        if (widgetsRepo.focused != widget) editor.getSelection().removeAllRanges();
        ...
        editor.on(
            ...
            function (evt) {
                ...
                if (!widget || Widget.getNestedEditable(widget.wrapper, evt.data.element)) return;
    ...
    function setupKeyboardObserver(widgetsRepo) {
        ...
        editor.on(
            ...
            function (evt) {
                ...
                else if (widgetHoldingFocusedEditable) ret = onEditableKey(widgetHoldingFocusedEditable, evt.data.keyCode);
    ...
    function setupSelectionObserver(widgetsRepo) {
        ...
        editor.on('selectionChange', function (evt) {
            ...
            if (oldWidget) {
                if (oldWidget !== newWidget || !oldWidget.focusedEditable.equals(nestedEditable)) {
                    ...
                    if (newWidget && nestedEditable) setFocusedEditable(widgetsRepo, newWidget, nestedEditable);
                }
            }
        ...
        editor.on('blur', function () {
            ...
            if ((widget = widgetsRepo.widgetHoldingFocusedEditable)) setFocusedEditable(widgetsRepo, widget, null);
        });
        ...
        editor.on(
            ...
            function (evt) {
                ...
                if (evt.data.protectedWhitespaces) {
                    ...
                    processedWidgetOnly =
                        evt.data.dataValue.children.length == 3 && Widget.isParserWidgetWrapper(evt.data.dataValue.children[1]);
                } else {
                    processedWidgetOnly =
                        evt.data.dataValue.children.length == 1 && Widget.isParserWidgetWrapper(evt.data.dataValue.children[0]);
                }
            },
            null,
            null,
    ...
    function setupUndoFilter(undoManager) {
        ...
        undoManager.addFilterRule(function (data) {
            return data
                ...
                .replace(/<span[^>]*cke_widget_drag_handler_container[^>]*.*?<\/span>/gim, '');
        });
    }
        ...
        function saveStyleGroup(style) {
            ...
            function getCompareFn(left) {
                ...
                function deepCompare(left, right) {
                    ...
                    for (var key in left) {
                        var areSameObjects =
                            typeof left[key] === 'object' && typeof right[key] === 'object' && deepCompare(left[key], right[key]);
